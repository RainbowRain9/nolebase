...
| --- | --- | --- | --- | --- | --- |
| 前端 | tabBar 主包骨架搭建 | 无 | 更新后的 app.json 主包配置与 pages/today、pages/diary、pages/treehole、pages/mine 空页面骨架 | 微信开发者工具可正常加载四个 Tab；路由与样式符合《分包规划方案》 | [分包规划方案](docs/分包规划方案.md)<br>[页面-今天](docs/页面文档/页面-今天.md) |
| 前端 | 公共卡片与按钮组件库初版 | tabBar 主包骨架搭建 | components 目录下的卡片、按钮、宫格基础组件 | 组件在今天与日记页复用不破版；通过设计走查并匹配《页面文档》示例 | [页面-今天](docs/页面文档/页面-今天.md)<br>[页面-日记](docs/页面文档/页面-日记.md) |
| 测试 | 公共组件可用性冒烟测试 | 公共卡片与按钮组件库初版 | Jest 与真机冒烟用例清单及测试报告 | 核心组件交互用例全部通过；发现缺陷均登记到缺陷库 | [测试用例](docs/测试用例.md) |

## 模块：今天（Home）
| 类型 | 任务名 | 前置依赖 | 产出物 | 验收标准 | 涉及文档链接 |
| --- | --- | --- | --- | --- | --- |
| 前端 | TopMoodCard 情绪打卡面板实现 | 公共卡片与按钮组件库初版 | TopMoodCard 组件与心情轮交互逻辑 | 真机完成情绪打卡流程；UI 与交互符合文档描述 | [页面-今天](docs/页面文档/页面-今天.md)<br>[功能-情绪打卡](docs/功能文档/功能-情绪打卡.md) |
...
| 运维 | 今天页监控与埋点告警配置 | 今天页主流程测试执行、今天与日记关键事件埋点集成 | Grafana/飞书告警配置、监控手册更新 | 关键指标（崩溃率、接口错误率、埋点漏斗）接入监控并触发测试告警 | [运维与监控](docs/运维与监控.md)<br>[埋点与数据分析](docs/埋点与数据分析.md) |

## 模块：日记
| 类型 | 任务名 | 前置依赖 | 产出物 | 验收标准 | 涉及文档链接 |
| --- | --- | --- | --- | --- | --- |
| 前端 | 日记与清单分段视图实现 | 公共卡片与按钮组件库初版 | JournalTabs 组件与列表骨架 | 分段切换在真机无闪烁；满足页面动效与布局要求 | [页面-日记](docs/页面文档/页面-日记.md) |
| 前端 | REBT 模板表单与附件输入 | 日记与清单分段视图实现 | REBT 表单组件、附件上传入口 | REBT A-E 表单可保存；附件选择与回显正常 | [功能-日记记录与REBT](docs/功能文档/功能-日记记录与REBT.md) |
| 数据库 | journals 集合索引与软删策略落地 | 无 | journals 索引与权限脚本、软删执行方案 | 索引建立成功；软删字段通过测试账号验证；权限表达式正确 | [集合-日记.journals](docs/数据库/集合-日记.journals.md) |
| 联调 | /api/journals 与清单接口联调 | REBT 模板表单与附件输入、journals 集合索引与软删策略落地、taskReceipt 云函数实现与写入校验 | /api/journals、/api/checklists 联调记录与 Swagger 更新 | 日记创建、查询、生成任务链路无错误；任务与清单同步无丢失 | [接口占位文档](docs/接口占位文档.md)<br>[集合-任务.tasks](docs/数据库/集合-任务.tasks.md) |
| 测试 | 日记模块专项测试 | /api/journals 与清单接口联调 | 测试报告、缺陷闭环记录 | 日记记录、过滤、模板、任务生成用例全部通过 | [测试用例](docs/测试用例.md) |
| 运维 | 日记数据备份与脱敏流程配置 | 日记模块专项测试 | 备份脚本、脱敏清单、执行排程 | journals 每日快照入 COS；脱敏脚本抽检通过 | [运维与监控](docs/运维与监控.md)<br>[集合-日记.journals](docs/数据库/集合-日记.journals.md) |
...
| 数据库 | users 与 reports 权限及索引核查 | 无 | users 与 reports 集合权限、索引更新记录 | 权限表达式测试账号验证；索引创建成功 | [集合-用户.users](docs/数据库/集合-用户.users.md)<br>[集合-周报与风控.reports](docs/数据库/集合-周报与风控.reports.md) |
...
| 类型 | 任务名 | 前置依赖 | 产出物 | 验收标准 | 涉及文档链接 |
| --- | --- | --- | --- | --- | --- |
| 前端 | SOS 面板与资源入口 UI 实现 | 公共卡片与按钮组件库初版 | independent/sos 页面、资源跳转入口 | 长按浮标秒开 SOS 面板；资源跳转正常 | [功能-SOS安全守护](docs/功能文档/功能-SOS安全守护.md) |
...
| 类型 | 任务名 | 前置依赖 | 产出物 | 验收标准 | 涉及文档链接 |
| --- | --- | --- | --- | --- | --- |
| 前端 | 埋点封装与事件枚举实现 | 公共卡片与按钮组件库初版 | analytics.ts 封装、事件常量 | 统一封装供各页面调用；代码通过 lint | [埋点与数据分析](docs/埋点与数据分析.md) |
| 前端 | 今天与日记关键事件埋点集成 | 埋点封装与事件枚举实现、Top3 微建议列表与一键任务交互、REBT 模板表单与附件输入 | 埋点调用实现、事件对照表 | pv_today、mood_checkin_submit、journal_entry_submit 等事件在真机可上报 | [埋点与数据分析](docs/埋点与数据分析.md) |
...
| 联调 | 埋点采集链路联调 | 今天与日记关键事件埋点集成、events 集合索引与权限配置 | 数据通路联调报告、ClickHouse 核对截图 | 关键事件数据在 ClickHouse 看板可见，偏差小于 5% | [埋点与数据分析](docs/埋点与数据分析.md) |
...
- 公共卡片与按钮组件库初版 → 日记与清单分段视图实现 → journals 集合索引与软删策略落地 → /api/journals 与清单接口联调 → 日记模块专项测试 → 日记数据备份与脱敏流程配置
