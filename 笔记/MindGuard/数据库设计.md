...
本草案基�?`docs/功能文档` 与《用户流程说明》所述核心场景，提炼微信小程序所需的概念级数据模型，用于后续详细设计与接口梳理。设计遵循“最小必要信息”“安全审计留痕”的原则，并兼顾情绪记录、任务闭环、社区守护与资源触达等业务主线�?

## 实体与关系概�?

| 核心�?| 关键�?| 主要关系说明 |
| --- | --- | --- |
| 用户与画�?| `users`、`user_profiles` | 用户为一切数据的主键；画像信息为任务推荐、微建议和个性化排序提供特征。|
| 情绪与记�?| `checkins`、`journals`、`daily_reports`、`weekly_summaries` | 情绪打卡与日记沉淀原始数据，按�?周汇总支撑趋势分析、周报与建议生成。|
| 建议与闭�?| `suggestion_sessions`、`micro_suggestions`、`behavior_tasks`、`task_events` | 微建议池与展示会话生成任务清单，事件流水追踪执行状态并回写情绪回执。|
| 社区与守�?| `forum_posts`、`action_cards`、`forum_reactions`、`risk_signals` | 树洞发帖与行动卡互通任务系统，风险信号触发守护流程并记录审计。|
| 紧急求�?| `sos_cases`、`sos_followups` | SOS 呼救记录风险等级、处置动作与回访情况，串联守护者协作。|
| 通知触达 | `notifications`、`user_events` | 系统推送与用户行为事件沉淀触达效果与埋点分析。|
| 安全与合�?| `audit_logs`、`consents` | 记录授权、撤销、SOS 联系等敏感操作，支撑隐私中心与合规审计。|
| 资源与反�?| `support_resources`、`resource_feedback` | 资源地图提供求助入口，反馈数据反哺运营优化。|
| 成长激�?| `badge_catalog`、`badge_events` | 徽章体系根据情绪、任务、社区数据计算进度与解锁记录。|
...
| `emotion_baseline` | Integer | 情绪基线�?-100），供建议排序。|
...
**关系**：用于生�?`daily_reports`、`weekly_summaries`，并回流�?`badge_events` 计算连续打卡等指标�?
...
**关系**：与 `behavior_tasks`（生成任务）、`daily_reports`（趋势）、`weekly_summaries`（复盘）�?`badge_events`（写作成就）互通�?
...
| `origin_reference_type` | Enum | 关联的来源实体类型（�?`suggestion`,`action_card`,`tool`）。|
...
**关系**：与 `task_events` 一对多；可�?`checkins` 引用作为情绪回执来源；来源字段连接社区、周报等模块�?
...
**关系**：支撑依从性分析、徽章解锁、周报指标；�?`suggestion_sessions`、`weekly_summaries` 间接关联�?
...
**关系**：生成的任务写入 `behavior_tasks`；刷新次数用于画�?(`user_profiles.adherence_score`) 调整�?
...
**关系**：作�?`suggestion_sessions` 的候选池，命中后生成 `behavior_tasks`；标签用�?`user_profiles.preference_tags` 学习�?
...
**关系**：聚�?`checkins`、`task_events`、`journals`；为 `weekly_summaries` 与情绪趋势图提供数据�?
...
**关系**：与 `behavior_tasks`（生成下一步任务）、`consents`（分享授权）�?`badge_events`（周报阅读成就）相关�?
...
| `action_card_id` | UUID | 若生成行动卡，关�?`action_cards`。|
| `created_at` | Timestamp | 创建时间。|
| `updated_at` | Timestamp | 最近更新时间。|
...
| `assigned_guardian_id` | UUID | 值班守护�?工作人员。|
| `created_at` | Timestamp | 触发时间。|
| `resolved_at` | Timestamp | 关闭时间。|
| `notes` | Text | 处理说明或转介情况。|

**关系**：驱动守护工作流，产�?`audit_logs` �?`consents`；高等级信号可关�?`support_resources` 推荐�?
...
| `assigned_guardian_id` | UUID | 值守�?辅导员。|
| `created_at` | Timestamp | 发起时间。|
| `resolved_at` | Timestamp | 关闭时间。|
| `notes` | Text | 记录处理摘要。|

**关系**：严重呼救同步写�?`risk_signals`；处理动作在 `sos_followups`、`audit_logs` 留痕，并可能推�?`notifications` 给相关人员�?
...
| `target_type` | String | 被操作对象类型（�?`consent`,`resource`,`post`）。|
...
**关系**：来源于 `behavior_tasks` 提醒、`weekly_summaries` 推送、`sos_cases` 告警；阅读行为写�?`user_events`。|
...
| `related_resource` | JSON | 若关联任�?帖子/资源，记录目�?ID 与类型。|
...
**关系**：依�?`checkins`、`task_events`、`forum_posts` 等数据计算；与前端徽章展示直接对应�?

## 关系说明

- **用户主导关系**：`users` 是所有业务表的外键核心；`user_profiles` 提供个性化推荐特征，驱�?`suggestion_sessions` 与任务排序�?
- **情绪数据链路**：`checkins` �?`journals` 共同沉淀情绪与标签，�?周汇总存�?`daily_reports`、`weekly_summaries`，再反馈给微建议、周报、徽章系统�?
- **建议与任务闭�?*：`micro_suggestions` 提供模板池，`suggestion_sessions` 将其匹配给用户并生成 `behavior_tasks`；`task_events` 记录执行轨迹，完成后触发 `checkins` 回执及徽章进度更新�?
- **社区互�?*：`forum_posts` 可生�?`action_cards`，被用户领取后写�?`behavior_tasks`；帖子风险与执行复盘通过 `risk_signals`、`sos_cases`、`audit_logs` 跟踪，确保树洞安全�?
- **安全守护**：`sos_cases` �?`risk_signals` 汇总来�?SOS、帖子、任务回执的预警，`sos_followups` 记录线下/线上协作，并要求 `consents`、`audit_logs` 留痕�?
- **通知与分�?*：`notifications` 负责任务提醒、周报推送与守护告警；用户互动沉淀�?`user_events`，支撑漏斗分析与触达策略优化�?
- **资源触达与反�?*：`support_resources` 为资源地图数据源，用户的联系操作写入 `audit_logs` �?`resource_feedback`，并可能关联 `risk_signals` 做后续跟进�?
- **成长激�?*：`badge_catalog` + `badge_events` 组合追踪用户在情绪、任务、社区互动上的成就，并可�?`weekly_summaries` 推荐下一步行动�?
