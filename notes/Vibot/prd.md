...
当前 Vibot 是基于 Vue Vben Admin 5.5.9 的现代化 monorepo 项目，具备完善的前端技术栈和模块化架构。三家公司在业务运营中面临信息分散、效率低下、数据分析不足等问题。通过集成 AI 技术，特别是 Dify 平台的对话式 AI 能力，可以显著提升企业运营效率、决策质量和创新能力。该项目将把现有的 Vibot 从一个基础的管理系统扩展为综合性的企业级 AI 协作平台。
...
| 2025-10-23 | 1.1  | 导航与交互重构（AI 融合式）版 | Vibot AI |
...
采用 pnpm workspace 管理的 Monorepo 结构，这与现有 Vibot 架构保持一致。这种结构便于包管理和代码共享，支持独立开发和部署。

工程基线：Node ≥ 20.10，pnpm ≥ 9.12；使用 Turbo 进行任务编排。请在 PR/发布前执行 `pnpm check` 与 `pnpm test:*` 相关检查，保持与仓库的 CI 规约一致。

### 服务架构

后端已全面迁移至 **Supabase**（托管 PostgreSQL 的 BaaS）。主要能力包括：

- **Supabase Database（PostgreSQL + RLS）**：托管所有核心业务数据，通过行级安全策略完成多租户隔离与权限控制。
- **Supabase Auth**：负责用户认证、会话管理以及 OAuth/SSO 集成，JWT 中携带 `company_id` 与 `role_keys`。
- **Supabase Storage**：存放审批附件、产品文档等文件资源，结合桶级策略限制访问。
- **Supabase Edge Functions（Deno）**：承载复杂业务逻辑、第三方集成回调（如钉钉）与定时任务。

所有业务 API 由 Supabase **PostgREST** 自动生成，前端与 BFF 通过 `@supabase/supabase-js` 访问数据；原有自建 Node.js/Express 微服务及 TypeORM/Prisma ORM 不再使用。

#### 多租户数据建模与 RLS 策略

- 建立 `companies`、`company_members`、`member_roles` 三张核心表，每张业务表都必须包含 `company_id uuid not null references companies(id)` 与 `created_by uuid references auth.users(id)` 字段，统一通过视图/函数暴露。
- RLS 统一模式：`auth.jwt()->>'company_id'` 决定当前上下文，所有 `SELECT/INSERT/UPDATE/DELETE` 策略均附带 `company_id` 比对，并结合 `company_members` 记录做 RBAC/ABAC 判定：
  ```sql
  create policy "tenant_read" on public.projects
    for select using (
  ...
  ```
  插入与更新策略分别通过 `with check` 约束 `company_id` 与 `role_keys`，并对跨公司操作强制 `security definer` 函数，避免服务端绕过 RLS。
- 为满足性能要求，所有策略用到的列（如 `company_id`、`role_keys`、`resource_id`）建立 B-Tree 索引，并在查询前附带显式过滤，遵循 Supabase RLS/性能调优指引，定期以 `EXPLAIN ANALYZE` 验证策略开销。

#### Supabase Auth / JWT 上下文传播

- 使用 Supabase Auth `app_metadata` 记录用户可访问的公司列表（`company_ids`）与角色矩阵（`role_keys`），`user_metadata.active_company_id` 表示当前上下文；切换公司时通过 `edge function switch-company` 校验 membership，并调用 `supabase.auth.updateUser()` 写入最新 `user_metadata` 后刷新 JWT。
- JWT 中除了默认 `role` claim，还注入 `company_id`、`role_keys`、`data_scopes`，Postgres 通过 `auth.jwt()` 读取。任何需要更低粒度权限的逻辑（例如字段级 ABAC）在策略中解析 JSON Claim，减少客户端校验分支。
- 会话续期、MFA、SSO（SAML/企业 IdP）仍由 Supabase Auth 承担，SSO 元数据在 `settings/security` 中配置，Edge Functions 仅负责生成一次性的登录魔法链接或邀请。

#### Edge Functions + ActionBus + Dify 安全代理

- Edge Functions 作为“AI 代理/BFF”，使用 `createClient(..., { global: { headers: { Authorization: req.headers.get('Authorization') } } })` 以用户上下文访问数据库，确保 RLS 生效；`service_role` 仅在管理任务和定时任务中使用。
- `functions/dify-webhook` 接收 PostMessage 的 HMAC，并将白名单字段写入 `ai_conversation_events`，再安全地调用 Dify/Dify 回调；`functions/action-bus` 在命令执行前二次校验 RBAC/ABAC、生成审计记录，并将动作排入队列或直接调用 PostgREST。
- 复杂第三方调用（OpenAI、钉钉、外部连接器）全部托管在 Edge Functions，近源运行避免泄露客户端密钥，并可以借助 Supabase Secrets 管理。

#### Storage 与附件安全

- 依据 Supabase Storage Access Control 建议，按公司维度创建私有桶（如 `company-${company_id}`）并强制 RLS：`bucket_id = format('company-%s', auth.jwt()->>'company_id')`；附件路径再细分 `audits/`, `contracts/`, `ai-cache/` 等前缀。
- 行为日志/审批附件仅允许拥有 `auditor` 或动作发起人下载，上传前通过 Edge Functions 生成预签名 URL，防止客户端直接持 service key。

#### Realtime 与命令面板通道

- Supabase Realtime 启用 `private` channel，并在 `realtime.messages` 上配置策略：`realtime.topic() = format('company:%s:conversation:%s', auth.jwt()->>'company_id', card_id)`，从而保证多租户隔离。
- 对话握手/命令面板同步使用 Realtime Broadcast + Presence，所有 PostMessage 回执都走 `conversation:<trace_id>` 频道，并在连接前完成权限校验与签名验证。

#### 性能、连接池与监控

- 前端与 Edge Functions 访问数据库时默认使用 Supavisor 事务池连接串（低延迟、高弹性）；后台批量或长连接任务使用 Session 模式或直连 IPv6，以平衡连接数。
- PostgREST 查询全部显式 `select`, `limit`, `order`, `Prefer: count=exact`，并通过缓存层（Edge Function + Redis/Cloudflare KV）保护热点接口；平台监控通过 Supabase Insights/Reports 叠加自定义告警，遵循性能调优文档。

#### AI 上下文缓存与 pgvector

- 利用 Supabase pgvector 能力构建 `ai_context_chunks`、`ai_conversation_threads` 表：`embedding vector(1536)`，结合 `company_id` RLS 做上下文缓存，命令面板/对话可就近读取缓存以满足 800ms 首 token 预算。
- Edge Functions 可选在同区域运行开源 embedding 模型或调用外部模型，将结果写入上述表，并在 Dify 回调时带上 `trace_id`、`nonce`、`ts` 形成可追溯链路。
...
- **BaaS 平台**：Supabase（Database + Auth + Storage + Edge Functions + PostgREST）
- **数据库**：Supabase 托管 PostgreSQL，启用 RLS／策略函数；多数据源通过 Edge Functions + 连接器访问
- **缓存/消息队列**：如需补充，可在 Edge Functions 或 BFF 层接入（例如 Cloudflare KV/Bull），并与 Supabase 相互配合
- **文件存储**：Supabase Storage 为主，按需同步到 OSS/S3 进行备份
...
- **Supabase 项目管理**：通过 Supabase Dashboard 与 CLI 进行迁移、Edge Functions 和权限策略的部署
- **BFF/前端**：继续使用 GitHub Actions 或 GitLab CI；如需容器化，主要针对 BFF/前端静态资源
- **监控与日志**：依托 Supabase Log Explorer、Insights，可按需对接第三方 APM/告警
- **日志**：结构化日志记录，支持日志聚合和分析

### 与现有代码的接口契约（首批）

项目正式后端 API 由 Supabase **PostgREST** 自动生成，所有请求通过 `https://<project-ref>.supabase.co/rest/v1/<table>` 访问，并遵循 `@supabase/supabase-js` 客户端或标准 PostgREST 过滤语法。详细实践方式与示例请参考最新的《Vibot Supabase API 实践手册》（docs/api-spec.md）。
...
- **Supabase 技术实现要点：**
  - 数据模型：`companies`（租户）、`company_members`（用户-公司关系，含 `role_keys`）、`company_switch_events`（上下文切换审计）。所有业务表引用 `company_id` 外键并建立联合索引 `(company_id, updated_at)`。
  - 权限实现：在 `company_members` 上建立 `select` 策略 `user_id = auth.uid()`，其余表策略引用该表校验 membership；跨公司访问必须通过 `rpc(request_cross_company_access)`，由 Edge Function 校验是否具备 `cross_company_viewer`。
  - 上下文切换：前端调用 Edge Function `switch-company`，函数校验 membership 并更新 `user_metadata.active_company_id`，刷新 JWT 后通过 Realtime Channel 广播导航/缓存刷新事件，保证各视图立即收敛。
...
- **Supabase 技术实现要点：**
  - 新建 `audit_logs` 表（`company_id`, `trace_id`, `message_id`, `resource`, `action`, `decision`, `payload`, `performed_by`, `performed_at`），并通过 `security definer` 函数 `log_action()` 暴露给 Edge Functions，确保任何调用都落表。
  - 字段级权限：敏感字段统一放入 `secure_attributes` 视图，由 `policies` 检查 `auth.jwt()->'role_keys'` 是否包含 `hr.secure.read`，并在响应中对无权限用户返回脱敏占位符。
  - 审计导出：通过 PostgREST 视图 `audit_logs_export` 暴露 CSV/JSON，利用 Storage 桶 `company-{id}-audits` 缓存导出文件，结合 RLS 限制仅安全角色可访问。
...
- **Supabase 技术实现要点：**
  - `functions/conversation-gateway`：校验来自前端的 HMAC（`traceId|nonce|ts`），创建带用户上下文的 Supabase Client，再调用 Dify/OpenAI。函数内通过 `auth.getUser(token)` 读取角色元数据并写入 `ai_conversation_events`。
  - PostMessage 安全：iframe 握手后在 Realtime 私有频道 `conversation:{company_id}:{thread_id}` 中交换令牌，借助 Realtime Authorization 策略限制非成员连接。
  - 退化策略：当 Dify 调用超时即写入 `ai_conversation_fallbacks`，并在 Edge Function 中返回缓存模板；缓存命中时直接读取 `ai_context_chunks`，满足首 token SLA。
...
- **Supabase 技术实现要点：**
  - `card_templates`（定义 CardSchema 版本）、`card_instances`（落地到对话流）、`card_actions`（ActionBus 元数据）三表，均携带 `company_id` 与 `schema_version`，并通过 `rpc(resolve_card_schema)` 统一读取。
  - ActionBus 由 Edge Function `action-dispatcher` 承担，函数内部根据 `card_actions.handler` 路由到 PostgREST / 外部 API，并写入 `audit_logs`；为避免重复执行，附带 `idempotency_key`。
  - 表单片段 JSON Schema 存储在 `card_form_fragments`，提交前由前端与 Edge Function 双重校验；需要持久化草稿时写入 `action_drafts`，并通过 RLS 确保仅发起人可查看/修改。
...
- **Supabase 技术实现要点：**
  - 命令与搜索条目标注在 `command_palette_entries`（含 `keywords`, `route`, `action_type`, `required_roles`），并通过 `pg_trgm` + `tsvector` + `vector` 三重索引实现语义/关键字混排。
  - `functions/command-router` 接管命令执行流程：先按 `required_roles` 验证，再调用相应 `rpc` 或 Edge Function；所有执行请求写入 `command_invocations`，便于审计与速率限制。
  - 高并发搜索通过 Supabase Realtime 推送“命令热度”以驱动前端排序，同时在缓存层维护 `company_id` 级别的热门命令列表，保证 200ms 以内的结果反馈。
...
**Stories（概览）：**
- 详见 Story 15.1–15.3（待拆分），覆盖订单创建、状态追踪、异常处理。

**兼容性要求：**
- 与审批、财务、项目模块共享订单主数据；支持多公司币种。

**风险与回滚：**
- 风险：订单与外部系统同步失败；缓解：重试与对账。回滚：保留手工导出导入。
...
### Epic 16: 生产工单与工序执行（MES）

**Epic 目标：** 构建订单驱动的工单管理与工序执行能力，实时掌握产量、质量与成本。

**Epic 描述（现状与增强）：**
- 现有：工序产量通过 Excel/纸质记录，难以及时汇总。
- 增强：从订单生成工单，提供工序上报、质量跟踪与异常预警。

**Stories（概览）：**
- 详见 Story 16.1–16.3（待拆分），覆盖工单排程、工序上报、质量反馈。

**兼容性要求：**
- 与设备数据、质量系统对接，支持移动端录入。

**风险与回滚：**
- 风险：工单数据与财务成本差异；缓解：定期对账与预警。回滚：保留手工导入入口。
